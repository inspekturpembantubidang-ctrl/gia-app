...
// ─── DOCX GENERATOR (template-matched) ───────────────────────────────────────
async function generateDocx(jenis: string, tanggal: string, desaPhotos: Record<string, string | null>) {
  ...
  // ── Dimensions (exact from template) ──
  // Page: A4 Portrait, margin 720 dxa all sides
  // Table width: 10605 dxa (from template tblW)
  // Col widths: 1545 | 3825 | 1575 | 3660
  const COL1 = 1545; // desa name left
  const COL2 = 3825; // photo left
  const COL3 = 1575; // desa name right
  const COL4 = 3660; // photo right
  const TBL_W = COL1 + COL2 + COL3 + COL4; // 10605

  // Photo size in EMU (from template wp:extent): ~2420000 x 1915000
  const FOTO_W = 2418715;
  const FOTO_H = 1914525;
  ...
  const zip = new JSZip();
  const imgRels: ({ rId: string; partName: string; mime: string } | null)[] = []; // one per desa, null if no photo
  let rIdCounter = 2; // rId1 = styles
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
          ...
          <w:r>
            ...
            <w:drawing>
              <wp:inline distT="114300" distB="114300" distL="114300" distR="114300"
                ...
                <wp:extent cx="${FOTO_W}" cy="${FOTO_H}"/>
                <wp:effectExtent l="0" t="0" r="635" b="9525"/>
                <wp:docPr id="${idx + 10}" name="Foto${idx + 1}"/>
                <wp:cNvGraphicFramePr/>
                <a:graphic xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">
                  <a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture">
                    <pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture">
                      <pic:nvPicPr>
                        <pic:cNvPr id="${idx + 10}" name="Foto${idx + 1}"/>
                      ...
                      <pic:spPr>
                        <a:xfrm><a:off x="0" y="0"/><a:ext cx="${FOTO_W}" cy="${FOTO_H}"/></a:xfrm>
  ...
  // ── Build rows: 2 desa per row, 4 rows total ──
  let tableRows = "";
  for (let row = 0; row < 4; row++) {
    const leftIdx = row * 2;       // 0, 2, 4, 6
    const rightIdx = row * 2 + 1;  // 1, 3, 5, 7 (7 doesn't exist → empty)

    const leftDesa  = DESAS[leftIdx]  || null;
    const rightDesa = DESAS[rightIdx] || null;
    const leftRel   = leftDesa  ? imgRels[leftIdx]  : null;
    const rightRel  = rightDesa ? imgRels[rightIdx] : null;

    tableRows += `
    <w:tr>
      <w:trPr><w:trHeight w:val="3005"/></w:trPr>
      ${leftDesa  ? makeNameCell(COL1, leftDesa)  : makeEmptyCell(COL1)}
      ${leftDesa  ? makePhotoCell(COL2, leftRel,  leftIdx)  : makeEmptyCell(COL2)}
      ${rightDesa ? makeNameCell(COL3, rightDesa) : makeEmptyCell(COL3)}
      ${rightDesa ? makePhotoCell(COL4, rightRel, rightIdx) : makeEmptyCell(COL4)}
    </w:tr>`;
  }

  // ── Header lines (DISUSUN / DIREVIU / DISETUJUI) with tab stops ──
...
function ApipPortal({ onBack }: { onBack: () => void }) {
  ...
  const openLightbox = (desa: string, url: string) => {
      ...
      <div className="app-shell">
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div className="status-card scale-in" style={{ maxWidth: 380, width: "100%" }}>
            ...
            <div className="status-sub" style={{ marginBottom: 24 }}>
              Masukkan password untuk mengakses portal pengawas internal.
            </div>
            ...
            >
              <span className="msymbol sm">login</span>
              Masuk Portal APIP
            </button>
          </div>
        </div>
                  ...
                  <div className="desa-card-header" onClick={() => setExpandedDesa(isOpen ? null : d)}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3, flexWrap: "wrap" }}>
                        ...
                        {PIC_DATA[d]?.hp && (
                          <a
                            href={`https://wa.me/62${PIC_DATA[d].hp.replace(/^0/, "")}`}
                            target="_blank"
